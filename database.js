...
// Type: 'PRO' or 'VIP'
function createVoucher(type, days, count = 1) {
    const db = getDB(); 
    const code = "BLAZE" + type + "-" + Math.random().toString(36).substring(2, 8).toUpperCase();
    db.vouchers[code] = { type, days, uses: count, usedBy: [] }; 
    saveDB(db); 
    return code;
}
...
function saveSessionMeta(sid, o, t) { const db = getDB(); db.sessionMeta[sid] = { owner: o, type: t, connectedAt: new Date().toISOString() }; saveDB(db); }
...
function generateApiKey(uid) {
    ...
    db.users[uid].apiKey = key; saveDB(db); return key;
}
function getUidByApiKey(k) {
    const db = getDB(); for (let id of Object.keys(db.users)) if (db.users[id].apiKey === k) return Number(id); return null;
}
...
module.exports = {
    ...
    generateApiKey, getUidByApiKey, setWebhook,
