...
// Optional: HttpsProxyAgent implementation can go here if proxy URL is added in .env
...
async function startSession(sessionId, displayName, requesterInfo, sessionType = "public", bot, silent = false) {
    ...
    try {
        ...
        let agent = undefined;
        if (dynConfig.ENABLE_PROXY && process.env.PROXY_URL) {
            // agent = new HttpsProxyAgent(process.env.PROXY_URL);
            console.log(`🛡️ [WA] Proxy enabled for session: ${sessionId}`);
        }
        ...
        sock.ev.on("connection.update", async (update) => {
            ...
            if (connection === "open") {
                state.setSession(sessionId, { status: "Connected", connectedAt: Date.now() });
                saveSessionMeta(sessionId, requesterInfo.id, sessionType);
