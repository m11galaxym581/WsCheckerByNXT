...
const { 
    getDB, saveDB, isAdmin, isVIP, isSub, isBanned, verifyWebPass, setWebhook, 
...
function startServer(bot) {
    ...
    app.get(["/", "/dashboard", "/checker", "/history", "/sessions", "/profile", "/api", "/webhooks", "/jobs", "/lists", "/templates", "/files", "/audit", "/plans", "/vouchers", "/system", "/docs", "/settings", "/support", "/admin", "/share/:id"], (req, res) => res.sendFile(path.join(__dirname, "index.html")));
    ...
    function requireCsrf(req, res, next) {
        if (req.path === "/api/login" || req.path.startsWith("/api/v1")) return next();
    ...
    function ensureFeatureDB(db) {
        ...
        if (!db.meta.referrals) db.meta.referrals = {};
    ...
    app.post("/api/login", (req, res) => {
        const { uid, password, humanToken, humanAnswer } = req.body; 
        const db = getDB(); 
        const nUid = Number(uid);
        ...
        if (!verifyHumanChallenge(humanToken, humanAnswer, req)) {
            recordLoginFailure(req, nUid);
            return res.status(400).json({ ok: false, message: "Human verification failed. Please try again." });
        }
    ...
    app.post("/api/user/generate-api", requireAuth, (req, res) => {
        ...
        if(key) res.json({ ok: true, apiKey: key }); 
    ...
    app.post("/api/api-key/secondary", requireAuth, (req,res)=>{ const {db,u}=userFeature(req.user.uid); const key='B3AST2-'+crypto.randomBytes(18).toString('base64url').toUpperCase(); u.apiKeys.unshift({key,createdAt:new Date().toISOString(),active:true}); saveDB(db); audit(req.user.uid,'api_key_secondary',req.user.uid); res.json({ok:true,key}); });
    ...
    app.post("/api/referral", requireAuth, (req,res)=>{ const db=ensureFeatureDB(getDB()); const code=crypto.createHash('sha1').update(String(req.user.uid)).digest('hex').slice(0,8).toUpperCase(); db.meta.referrals[code]={owner:req.user.uid,createdAt:new Date().toISOString()}; saveDB(db); res.json({ok:true,code,link:`/dashboard?ref=${code}`}); });
...
┣ 🆔 ID: \`${uid}\``, {
    ...
    app.get("/api/admin/stats/:adminUid", requireAdmin, (req, res) => {
        res.json({ 
            ...
            ...getStats(),
            systemMode: String(config.dynamic.SYSTEM_MODE || "subscription").toLowerCase(), 
            nodeHealth: state.nodeHealth || {}, nodeMetrics: state.nodeMetrics || {}, 
